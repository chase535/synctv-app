...
void main() {
  group('official-site login targets', () {
    test('iQIYI opens its dedicated official login surface', () {
      ...
      expect(officialSiteLoginProviderName(WebPlaybackProvider.iqiyi), '爱奇艺');
    });

    test(
      'Tencent opens its official video site for current account login UI',
      () {
        expect(
    ...
    test(
      'all login entry points are HTTPS and contain no embedded credentials',
      () {
